...
public class DailyReportScheduler {
    ...
    public DailyReportScheduler(OrderRepository orderRepo, EmailService emailService,
            OrderItemRepository orderItemRepo, MenuRepository menuRepository) {
    ...
    /* ================= DAILY INVENTORY RESET (MIDNIGHT) ================= */
    @Scheduled(cron = "0 0 0 * * *")
    public void resetDailyInventory() {
        System.out.println("Resetting daily inventory...");
        java.util.List<MenuItem> items = menuRepository.findAll();
        for (MenuItem item : items) {
            if (!Boolean.TRUE.equals(item.getUnlimited())) {
                item.setRemainingQuantity(item.getDailyLimit());
                item.setAvailable(true); // re-enable auto-disabled items
            }
        }
        menuRepository.saveAll(items);
        System.out.println("Daily inventory reset complete for " + items.size() + " items.");
    }

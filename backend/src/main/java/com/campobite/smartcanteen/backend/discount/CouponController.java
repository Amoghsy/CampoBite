...
public class CouponController {
    ...
    public Coupon updateCoupon(@PathVariable Long id, @RequestBody Coupon details) {
        ...
        // send an updated offer notification if the coupon is active
        if (saved.isActive()) {
            notificationService.sendTopicNotification(
                    "coupons",
                    "Offer Updated! 🎯",
                    "Code " + saved.getCode() + " now gives " + saved.getDiscountPercentage() + "% off",
                    java.util.Map.of("code", saved.getCode()));
        }
    ...
    public ResponseEntity<?> deleteCoupon(@PathVariable Long id) {
        // read code before deletion for message
        couponRepository.findById(id).ifPresent(c -> {
            notificationService.sendTopicNotification(
                    "coupons",
                    "Offer Removed",
                    "Coupon " + c.getCode() + " is no longer available",
                    java.util.Map.of("code", c.getCode()));
        });

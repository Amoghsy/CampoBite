...
public class NotificationService {
    ...
    /**
     * Send a generic data message directly to a single FCM token.
     */
    @Async
    public void sendTokenNotification(String fcmToken, String title, String body, java.util.Map<String, String> data) {
        if (fcmToken == null || fcmToken.isBlank()) {
            System.err.println("❌ Token notification failed: token is null/empty");
            return;
        }
        try {
            ...
            System.out.println("🔥 FCM Token Sent: " + response + " to " + fcmToken);
        } catch (Exception e) {
            System.err.println("❌ FCM Token notification failed");
            e.printStackTrace();
        }
    }
}

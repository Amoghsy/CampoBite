...
export default function Dashboard() {
  ...
  const placeOrder = async () => {
    ...
    // Step 1: Pre-create a Razorpay order (server-side) to get an orderId for webhook reconciliation
    try {
      ...
      console.warn('Could not pre-create Razorpay order. Continuing without orderId.', err);
    }
    ...
    const options = {
      ...
      order_id: razorpayOrderId ?? undefined, // links payment to pre-created order
      ...
      handler: async (response: any) => {
        try {
          ...
          const res = await api.post("/api/orders", {
            ...
            // 🔑 Pass Razorpay orderId so webhook can update paymentStatus
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                {activeOrder.paymentStatus && (
                                  <Badge className={`text-[10px] px-1.5 py-0 border ${activeOrder.paymentStatus === 'CAPTURED' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
                                    activeOrder.paymentStatus === 'REFUNDED' ? 'bg-blue-100 text-blue-700 border-blue-200' :
                                      activeOrder.paymentStatus === 'FAILED' ? 'bg-red-100 text-red-700 border-red-200' :
                                        'bg-amber-100 text-amber-700 border-amber-200'
                                    }`}>
                                    {activeOrder.paymentStatus === 'CAPTURED' ? '💳 Paid' :
                                      activeOrder.paymentStatus === 'REFUNDED' ? '↩ Refunded' :
                                        activeOrder.paymentStatus === 'FAILED' ? '✗ Failed' :
                                          '⏳ Pending'}
                                  </Badge>
                                )}
              ...
              <TabsContent value="menu" className="space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  {filteredItems.map(item => {
                    // Treat as out-of-stock if disabled OR if non-unlimited item has 0 remaining
                    const isOutOfStock = !item.available ||
                      (!item.unlimited && item.remainingQuantity !== undefined && item.remainingQuantity !== null && item.remainingQuantity <= 0);

                    return (
                      <Card
                        key={item.id}
                        className={`menu-card overflow-hidden h-full flex flex-col ${isOutOfStock ? 'opacity-60 grayscale-[30%]' : ''}`}
                      >
                          ...
                          <div key={o.id} className="p-5 rounded-2xl bg-muted/30 border border-border/50 space-y-4 hover:border-accent/30 transition-colors">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                              ...
                              <div className="flex items-center justify-between sm:block text-right">
                                ...
                                {o.paymentStatus && (
                                  <Badge className={`text-[10px] px-1.5 py-0 border mt-1 ${o.paymentStatus === 'CAPTURED' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
                                    o.paymentStatus === 'REFUNDED' ? 'bg-blue-100 text-blue-700 border-blue-200' :
                                      o.paymentStatus === 'FAILED' ? 'bg-red-100 text-red-700 border-red-200' :
                                        'bg-amber-100 text-amber-700 border-amber-200'
                                    }`}>
                                    {o.paymentStatus === 'CAPTURED' ? '💳 Paid' :

...
export function Chatbot() {
  ...
  const handleSend = async (messageOverride?: string) => {
    ...
    try {
      ...
      // If backend indicates a chat-based order that should open Razorpay, trigger checkout
      if (data.action === "OPEN_CHECKOUT" && data.amount && data.menuItemId && data.quantity) {
        // @ts-ignore Razorpay is loaded globally (same as dashboard)
        ...
        try {
          // Pre-create Razorpay order on backend to get orderId
          ...
          const options = {
            ...
            description: data.itemName
              ? `Chatbot order: ${data.quantity} x ${data.itemName}`
              : "Chatbot order",
            order_id: orderId,
            handler: async (response: any) => {
              try {
                const itemNames = data.itemName
                  ? `${data.itemName} x${data.quantity}`
                  : `Item #${data.menuItemId} x${data.quantity}`;
                ...
                console.error("Failed to create order after chatbot payment", e);
              }
            },
          };
          ...
          console.error("Chatbot Razorpay checkout failed", e);
        }
      }
    } catch (error) {
